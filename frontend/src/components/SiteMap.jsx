...
export default function SiteMap({ slopes }) {
  ...
  useEffect(() => {
    ...
    // faint terrain relief so the dark map reads as topography, not a flat void
    L.tileLayer(
      "https://server.arcgisonline.com/ArcGIS/rest/services/Elevation/World_Hillshade/MapServer/tile/{z}/{y}/{x}",
      { maxZoom: 16, opacity: 0.16 }
    ).addTo(map);
    L.control.attribution({ prefix: false }).addAttribution("© OpenStreetMap · CARTO · Esri").addTo(map);
  ...
  useEffect(() => {
    ...
    layer.clearLayers();
    slopes.forEach((s) => {
      const col = pal.marker[s.status] || pal.marker.normal;
      if (s.status !== "normal") {
